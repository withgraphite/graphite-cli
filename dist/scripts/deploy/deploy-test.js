...
function testDeploy() {
    ...
    (0, build_1.build)();
    console.log("zipping...");
    (0, child_process_1.execSync)(`zip screenplay-cli.zip . -r`, { stdio: "ignore" });
    const shasum = (0, child_process_1.execSync)("shasum -a 256 screenplay-cli.zip")
        .toString()
        .trim()
        .split(" ")[0];
    ...
    fs_extra_1.default.writeFileSync(formulaPath, fs_extra_1.default
        ...
        .replace("<sha>", shasum));
    (0, child_process_1.execSync)(`brew install --build-from-source ${formulaPath}`, {
        stdio: "inherit",
    });
    ...
    // expect not to fail...
    (0, child_process_1.execSync)(`${(0, child_process_1.execSync)("brew --prefix graphite").toString().trim()}/bin/gt stacks`, {
        stdio: "inherit",
    });
}
testDeploy();
