...
const survey_config_1 = require("../lib/config/survey_config");
...
async function postSurveyResponse() {
    try {
        const surveyConfig = survey_config_1.surveyConfigFactory.loadIfExists();
        const surveyResponse = surveyConfig?.data.responses;
        const authToken = user_config_1.userConfigFactory.loadIfExists()?.data.authToken;
        if (!surveyConfig || !surveyResponse || !authToken) {
            return;
        }
        const response = await retyped_routes_1.request.requestWithArgs(server_1.API_SERVER, graphite_cli_routes_1.default.surveyResponse, {
            authToken: authToken,
            responses: {
