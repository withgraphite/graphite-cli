...
function findCommonlyNamedTrunk(context) {
    const potentialTrunks = branch_1.default.allBranches(context).filter((b) => ['main', 'master', 'development', 'develop'].includes(b.name));
...
function getTrunk(context) {
    ...
    if (trunkSiblings.length > 0) {
        throw new errors_1.SiblingBranchError([memoizedTrunk].concat(trunkSiblings), context);
    }
    return memoizedTrunk;
}
