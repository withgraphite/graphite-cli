...
function setDefaultEditor() {
    const editor = (0, exec_sync_1.gpExecSync)({ command: `echo \${GIT_EDITOR:-$EDITOR}` }).toString().trim() ||
        editor_1.DEFAULT_GRAPHITE_EDITOR;
    config_1.userConfig.setEditor(editor);
}
...
function setDefaultEditorOrPrompt() {
    return __awaiter(this, void 0, void 0, function* () {
        if (!config_1.userConfig.getEditor()) {
            ...
            const systemEditor = (0, exec_sync_1.gpExecSync)({ command: `echo \${GIT_EDITOR:-$EDITOR}` })
                .toString()
                .trim();
            let editorPref;
            if (systemEditor.length) {
                editorPref = systemEditor;
                (0, splog_1.logTip)(`Graphite will now use ${editorPref} as the default editor setting. 
      ...
      If you wish to change it, use \`gt user editor\` to change this in the future`);
                ...
                (0, splog_1.logInfo)(chalk_1.default.yellow(`We did not detect an editor preference in your settings. Do you wish to set it? (Graphite will use ${editor_1.DEFAULT_GRAPHITE_EDITOR} as default.)`));
                const yesOrNo = yield (0, prompts_1.default)({
                    type: 'select',
                    name: 'editorPrompt',
            ...
            (0, splog_1.logInfo)(chalk_1.default.yellow(`Graphite editor preference set to ${editorPref}.`));
        }
    });
}
