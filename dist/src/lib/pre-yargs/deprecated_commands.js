...
exports.handleDeprecatedCommandNames = void 0;
...
function handleDeprecatedCommandNames(command) {
    switch (command[0]) {
        case 'branch':
        case 'b':
            if (['next', 'n', 'previous', 'p'].includes(command[1])) {
                (0, splog_1.composeSplog)().error(NAV_WARNING);
                // eslint-disable-next-line no-restricted-syntax
                process.exit(1);
            }
            if ('show' === command[1]) {
                (0, splog_1.composeSplog)().error(INFO_WARNING);
                // eslint-disable-next-line no-restricted-syntax
                process.exit(1);
            }
            break;
        ...
        case 'ds':
            if ('sync' === command[1]) {
                (0, splog_1.composeSplog)().error(GET_WARNING);
                // eslint-disable-next-line no-restricted-syntax
                process.exit(1);
            }
            break;
        ...
        case 's':
            if (['fix', 'f'].includes(command[1])) {
                (0, splog_1.composeSplog)().warn(FIX_WARNING);
            }
    }
}
exports.handleDeprecatedCommandNames = handleDeprecatedCommandNames;
//# sourceMappingURL=deprecated_commands.js.map