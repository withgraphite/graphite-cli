...
function composeMetaCache({ trunkName, currentBranchOverride, splog, noVerify, remote, }) {
    ...
    return {
        ...
        persist() {
            (0, persist_cache_1.persistCache)(trunkName, cache.branches, splog);
        },
        clear() {
            (0, persist_cache_1.clearPersistedCache)(splog);
        },
        rebuild(newTrunkName) {
            trunkName = newTrunkName ?? trunkName;
            cache.branches = (0, cache_loader_1.loadCachedBranches)({ trunkName, ignorePersistedCache: true }, splog);
        },
        get trunk() {
            return assertTrunk();
        },
