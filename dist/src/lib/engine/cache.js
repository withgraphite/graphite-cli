...
function composeMetaCache({ trunkName, currentBranchOverride, splog, noVerify, remote, restackCommitterDateIsAuthorDate, }) {
    ...
    const isViableParent = (branchName, parentBranchName) => {
        ...
        const mergeBase = (0, merge_base_1.getMergeBase)(branchName, parentBranchName);
        return (
    ...
    return {
        ...
        trackBranch: (branchName, parentBranchName) => {
            if (!isViableParent(branchName, parentBranchName)) {
                throw new errors_1.PreconditionsFailedError(`Can't track ${branchName} with parent ${parentBranchName}`);
            }
            updateMeta(branchName, {
