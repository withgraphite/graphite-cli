...
function captureState() {
    const refTree = (0, branch_relations_1.getRevListGitTree)({
        useMemoizedResults: false,
        direction: 'parents',
    });
    const branchToRefMapping = (0, branch_ref_1.getBranchToRefMapping)();
    const metadata = {};
    metadata_ref_1.default.allMetadataRefs().forEach((ref) => {
        metadata[ref._branchName] = JSON.stringify(ref.read());
    });
    const currentBranchName = (0, preconditions_1.currentBranchPrecondition)().name;
    const state = {
        refTree,
        branchToRefMapping,
...
function recreateState(stateJson) {
    ...
    (0, utils_1.logInfo)(`Creating ${Object.keys(state.refTree).length} commits`);
    recreateCommits({ refTree: state.refTree, refMappingsOldToNew });
    (0, utils_1.logInfo)(`Creating ${Object.keys(state.branchToRefMapping).length} branches`);
    createBranches({
        branchToRefMapping: state.branchToRefMapping,
        refMappingsOldToNew,
    });
    (0, utils_1.logInfo)(`Creating the repo config`);
    fs_extra_1.default.writeFileSync(path_1.default.join(tmpDir, '/.git/.graphite_repo_config'), state.repoConfig);
    (0, utils_1.logInfo)(`Creating the metadata`);
    createMetadata({ metadata: state.metadata, tmpDir });
    (0, utils_1.gpExecSync)({ command: `git checkout "${state.currentBranchName}"` });
    (0, utils_1.gpExecSync)({ command: `git branch -D "${tmpTrunk}"` });
    return tmpDir;
}
...
function createMetadata(opts) {
    ...
    Object.keys(opts.metadata).forEach((branchName) => {
        const metaSha = (0, utils_1.gpExecSync)({
...
function createBranches(opts) {
    const curBranch = (0, preconditions_1.currentBranchPrecondition)();
    Object.keys(opts.branchToRefMapping).forEach((branch) => {
        ...
        if (branch != curBranch.name) {
            (0, utils_1.gpExecSync)({ command: `git branch -f ${branch} ${originalRef}` });
        }
        else {
            (0, utils_1.logWarn)(`Skipping creating ${branch} which matches the name of the current branch`);
        }
    });
}
function recreateCommits(opts) {
    ...
    const firstCommitRef = (0, utils_1.gpExecSync)({ command: `git rev-parse HEAD` });
    ...
    while (commitsToCreate.length > 0) {
        ...
        const newCommitRef = (0, utils_1.gpExecSync)({
...
function createTmpGitDir(opts) {
    ...
    (0, utils_1.logInfo)(`Creating tmp repo`);
