...
function syncPRInfoForBranchByName(branchNames, context) {
    return __awaiter(this, void 0, void 0, function* () {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (response._response.status === 200) {
            ...
            response.prs.forEach((pr) => {
                var _a;
                const branch = branch_1.Branch.branchWithName(pr.headRefName, context);
                branch.upsertPRInfo({
                    number: pr.prNumber,
                    base: pr.baseRefName,
                    url: pr.url,
                ...
                if (branch.name !== pr.headRefName) {
                    splog_1.logError(`PR ${pr.prNumber} is associated with ${pr.headRefName} on GitHub, but branch ${branch.name} locally. Please rename the local branch (\`gt branch rename\`) to match the remote branch associated with the PR. (While ${branch.name} is misaligned with GitHub, you cannot use \`gt submit\` on it.)`);
                }
            });
        }
    });
