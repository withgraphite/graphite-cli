        ...
        if (response._response.status === 200) {
            ...
            yield Promise.all(response.prs.map((pr) => __awaiter(this, void 0, void 0, function* () {
                ...
                if (branch.name !== pr.headRefName) {
                    (0, utils_1.logError)(`PR ${pr.prNumber} is associated with ${pr.headRefName} on GitHub, but branch ${branch.name} locally. Please rename the local branch (\`gt branch rename\`) to match the remote branch associated with the PR. (While ${branch.name} is misaligned with GitHub, you cannot use \`gt submit\` on it.)`);
                }
            })));
        }
    });
