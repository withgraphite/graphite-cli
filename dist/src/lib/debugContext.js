...
function recreateState(stateJson, context) {
    ...
    splog_1.logInfo(`Creating ${Object.keys(state.refTree).length} commits`);
    recreateCommits({ refTree: state.refTree, refMappingsOldToNew });
    splog_1.logInfo(`Creating ${Object.keys(state.branchToRefMapping).length} branches`);
    createBranches({
        branchToRefMapping: state.branchToRefMapping,
        refMappingsOldToNew,
    }, context);
    splog_1.logInfo(`Creating the repo config`);
    fs_extra_1.default.writeFileSync(path_1.default.join(tmpDir, '/.git/.graphite_repo_config'), state.repoConfig);
    splog_1.logInfo(`Creating the metadata`);
    createMetadata({ metadata: state.metadata, tmpDir });
    exec_sync_1.gpExecSync({ command: `git checkout "${state.currentBranchName}"` });
    exec_sync_1.gpExecSync({ command: `git branch -D "${tmpTrunk}"` });
    return tmpDir;
}
...
function createBranches(opts, context) {
    ...
    Object.keys(opts.branchToRefMapping).forEach((branch) => {
        ...
        if (branch != curBranch.name) {
            ...
            splog_1.logWarn(`Skipping creating ${branch} which matches the name of the current branch`);
        }
    });
}
...
function createTmpGitDir(opts) {
    ...
    splog_1.logInfo(`Creating tmp repo`);
