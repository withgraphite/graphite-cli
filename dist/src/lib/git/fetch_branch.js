...
function fetchBranch(remote, branchName) {
    (0, exec_sync_1.gpExecSync)({
        command: `git fetch --no-write-fetch-head -f  ${(0, escape_for_shell_1.q)(remote)} ${(0, escape_for_shell_1.q)(branchName)}:${FETCH_HEAD}`,
        options: { stdio: 'pipe' },
        onError: 'throw',
    });
}
...
function writeFetchBase(sha) {
    (0, exec_sync_1.gpExecSync)({
        command: `git update-ref ${FETCH_BASE} ${(0, escape_for_shell_1.q)(sha)}`,
        options: { stdio: 'pipe' },
        onError: 'throw',
    });
}
