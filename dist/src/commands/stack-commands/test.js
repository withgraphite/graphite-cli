...
function testStack(command, opts) {
    const originalBranch = (0, preconditions_1.currentBranchPrecondition)();
    validateStack();
    (0, utils_1.logInfo)(chalk_1.default.grey(`Getting stack...`));
    const stack = new wrapper_classes_1.GitStackBuilder().fullStackFromBranch(originalBranch);
    (0, utils_1.logInfo)(chalk_1.default.grey(stack.toString() + '\n'));
    // Get branches to test.
    const branches = stack.branches().filter((b) => {
        if (opts.skipTrunk && b.name == (0, utils_1.getTrunk)().name) {
            return false;
        }
        return true;
    });
    ...
    (0, utils_1.logInfo)(chalk_1.default.grey(`Writing results to ${outputPath}\n`));
    ...
    // Finish off.
    (0, utils_1.checkoutBranch)(originalBranch.name);
}
function testBranch(opts) {
    (0, utils_1.checkoutBranch)(opts.branchName);
    ...
    const output = (0, utils_1.gpExecSync)({ command: opts.command, options: { stdio: 'pipe' } }, () => {
...
function logState(state, refresh) {
    ...
    Object.keys(state).forEach((branchName) => {
        ...
        (0, utils_1.logInfo)(`- ${color(state[branchName].status)}: ${branchName}${duration ? ` (${duration})` : ''}`);
    });
}
function validateStack() {
    try {
        (0, validate_1.validate)('FULLSTACK');
    }
    catch (err) {
