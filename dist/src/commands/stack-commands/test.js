...
function testStack(context, command, opts) {
    ...
    splog_1.logInfo(chalk_1.default.grey(`Getting stack...`));
    const stack = new git_stack_builder_1.GitStackBuilder().fullStackFromBranch(originalBranch, context);
    splog_1.logInfo(chalk_1.default.grey(stack.toString() + '\n'));
    ...
    splog_1.logInfo(chalk_1.default.grey(`Writing results to ${outputPath}\n`));
...
function logState(state, refresh) {
    ...
    Object.keys(state).forEach((branchName) => {
        ...
        splog_1.logInfo(`- ${color(state[branchName].status)}: ${branchName}${duration ? ` (${durationString})` : ''}`);
    });
}
