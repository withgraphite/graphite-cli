...
function testStack(context, command, opts) {
    ...
    const stack = new wrapper_classes_1.GitStackBuilder().fullStackFromBranch(originalBranch, context);
    utils_1.logInfo(chalk_1.default.grey(stack.toString() + '\n'));
    // Get branches to test.
    const branches = stack.branches().filter((b) => {
        if (opts.skipTrunk && b.name == utils_1.getTrunk(context).name) {
            return false;
        }
        return true;
    });
    ...
    // Finish off.
    utils_1.checkoutBranch(originalBranch.name, { quiet: true });
}
...
function logState(state, refresh) {
    ...
    Object.keys(state).forEach((branchName) => {
        ...
        utils_1.logInfo(`- ${color(state[branchName].status)}: ${branchName}${duration ? ` (${durationString})` : ''}`);
    });
}
