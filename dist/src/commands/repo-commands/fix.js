...
function branchCountSanityCheck(context) {
    ...
    if (branchCount > 50) {
        ...
        splog_1.logTip(`To further reduce Graphite's search space, you can also tune the maximum days and/or stacks Graphite tracks behind trunk using \`gt repo max-days-behind-trunk --set\` or \`gt repo max-stacks-behind-trunk --set\`.`, context);
        splog_1.logNewline();
    }
}
function deleteMergedBranchesContinuation() {
    splog_1.logNewline();
    splog_1.logInfo(`Still seeing issues with Graphite? Send us feedback via \`gt feedback '<your_issue'> --with-debug-context\` and we'll dig in!`);
}
