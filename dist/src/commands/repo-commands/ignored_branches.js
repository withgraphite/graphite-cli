    ...
    return telemetry_1.profile(argv, exports.canonical, () => __awaiter(void 0, void 0, void 0, function* () {
        if (argv.add) {
            ...
            if (foundBranches.length) {
                utils_1.logInfo(chalk_1.default.gray(`The following branches were found matching your pattern:`));
                foundBranches.split('/n').forEach((branch) => {
                    utils_1.logInfo(chalk_1.default.gray(branch.trim()));
                });
            }
            else {
                utils_1.logWarn(`No branches were found matching the provided pattern. Please make sure it is correct.`);
            }
            ...
            if (config_1.repoConfig.getIgnoreBranches().includes(argv.remove)) {
                ...
                utils_1.logInfo(`Removed pattern (${argv.remove}) from ignore list`);
            }
            else {
                utils_1.logInfo(`No pattern matching (${argv.remove}) found.`);
            }
        }
        else {
            const ignoredBranches = config_1.repoConfig.getIgnoreBranches();
            if (ignoredBranches.length) {
                utils_1.logInfo(`The following patterns are being ignored by Graphite:`);
                utils_1.logInfo(ignoredBranches.join('\n'));
            }
            else {
                utils_1.logInfo('No ignored branches');
            }
        }
    }));
});
...
function findMatches(branchName) {
    return utils_1.gpExecSync({ command: `git branch --list '${branchName}'` })
        .toString()
        .trim();
}
