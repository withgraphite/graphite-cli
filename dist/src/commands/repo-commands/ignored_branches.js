    ...
    return profile_1.profile(argv, exports.canonical, (context) => __awaiter(void 0, void 0, void 0, function* () {
        if (argv.add) {
            ...
            if (foundBranches.length) {
                context.splog.logInfo(chalk_1.default.gray(`The following branches were found matching your pattern:`));
                foundBranches.split('\n').forEach((branch) => {
                    context.splog.logInfo(chalk_1.default.gray(branch.trim()));
                });
            }
            else {
                context.splog.logWarn(`No branches were found matching the provided pattern. Please make sure it is correct.`);
            }
            ...
            if (context.repoConfig.getIgnoreBranches().includes(argv.remove)) {
                ...
                context.splog.logInfo(`Removed pattern (${argv.remove}) from ignore list`);
            }
            else {
            ...
            if (ignoredBranches.length) {
                context.splog.logInfo(`The following patterns are being ignored by Graphite:`);
