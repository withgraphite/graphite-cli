...
class MetadataRef {
    ...
    static branchMetadataDirPath() {
        return path_1.default.join((0, config_1.getRepoRootPath)(), `refs/branch-metadata/`);
    }
    ...
    static updateOrCreate(branchName, meta) {
        const metaSha = (0, child_process_1.execSync)(`git hash-object -w --stdin`, {
            input: JSON.stringify(meta),
        }).toString();
        (0, child_process_1.execSync)(`git update-ref refs/branch-metadata/${branchName} ${metaSha}`, {
            stdio: 'ignore',
        });
    }
    getPath() {
    ...
    read() {
        try {
            const metaString = (0, child_process_1.execSync)(`git cat-file -p refs/branch-metadata/${this._branchName} 2> /dev/null`)
                .toString()
                .trim();
