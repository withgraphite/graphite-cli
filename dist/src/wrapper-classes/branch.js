...
class Branch {
    ...
    getParentFromMeta() {
        var _a, _b;
        if (this.name === (0, utils_1.getTrunk)().name) {
            return undefined;
        }
    ...
    getChildrenFromMeta() {
        var _a;
        (0, utils_1.logDebug)(`Meta Children (${this.name}): start`);
        if (this.shouldUseMemoizedResults) {
            if (memoizedMetaChildren === undefined) {
                (0, utils_1.logDebug)(`Meta Children (${this.name}): initialize memoization | finding all branches...`);
                const metaChildren = {};
                const allBranches = Branch.allBranches({
                    useMemoizedResults: this.shouldUseMemoizedResults,
                });
                (0, utils_1.logDebug)(`Meta Children: intiialize memoization | sifting through branches...`);
                allBranches.forEach((branch, i) => {
                    var _a;
                    (0, utils_1.logDebug)(`               Branch ${i}/${allBranches.length} (${branch.name})`);
                ...
                (0, utils_1.logDebug)(`Meta Children (${this.name}): initialize memoization | done`);
                memoizedMetaChildren = metaChildren;
            }
            (0, utils_1.logDebug)(`Meta Children (${this.name}): end (memoized)`);
        ...
        (0, utils_1.logDebug)(`Git Children (${this.name}): end`);
        return children;
    }
    isUpstreamOf(commitRef) {
        const downstreamRef = (0, utils_1.gpExecSync)({
    ...
    ref() {
        return (0, git_refs_1.getRef)(this);
    }
    getMetaMergeBase() {
        ...
        const curParentMergeBase = (0, child_process_1.execSync)(`git merge-base ${curParentRef} ${this.name}`)
            .toString()
            .trim();
        if (!prevParentRef) {
            return curParentMergeBase;
        }
        const prevParentMergeBase = (0, child_process_1.execSync)(`git merge-base ${prevParentRef} ${this.name}`)
            .toString()
            .trim();
        ...
        const closestMergeBase = (0, child_process_1.execSync)(`git merge-base ${prevParentMergeBase} ${curParentMergeBase}`)
            .toString()
    ...
    static exists(branchName) {
        try {
            (0, child_process_1.execSync)(`git show-ref --quiet refs/heads/${branchName}`, {
                stdio: 'ignore',
            });
        }
        catch (_a) {
    ...
    getCurrentRef() {
        return (0, child_process_1.execSync)(`git rev-parse ${this.name}`).toString().trim();
    }
    clearMetadata() {
        this.writeMeta({});
        return this;
    ...
    lastCommitTime() {
        return parseInt((0, utils_1.gpExecSync)({ command: `git log -1 --format=%ct ${this.name} --` })
            .toString()
            .trim());
    }
    isTrunk() {
        return this.name === (0, utils_1.getTrunk)().name;
    }
    ...
    static getCurrentBranch() {
        const name = (0, utils_1.gpExecSync)({
    ...
    static allBranchesImpl(opts) {
        ...
        return (0, child_process_1.execSync)(`git for-each-ref --format='%(refname:short)' ${sortString} refs/heads/`)
            .toString()
            .trim()
            .split('\n')
            .filter((name) => name.length > 0 && !config_1.repoConfig.branchIsIgnored(name))
    ...
    static allBranchesWithFilter(args) {
        ...
        if (maxDaysBehindTrunk) {
            const trunkUnixTimestamp = parseInt((0, utils_1.getCommitterDate)({
                revision: (0, utils_1.getTrunk)().name,
        ...
        for (let i = 0; i < branches.length; i++) {
            ...
            if (minUnixTimestamp !== undefined) {
                const committed = parseInt((0, utils_1.getCommitterDate)({
    ...
    static getAllBranchesWithoutParents(opts) {
        return __awaiter(this, void 0, void 0, function* () {
            return this.allBranchesWithFilter({
                filter: (branch) => {
                    if ((opts === null || opts === void 0 ? void 0 : opts.excludeTrunk) && branch.name === (0, utils_1.getTrunk)().name) {
                        return false;
                    }
    ...
    head() {
        return new commit_1.default((0, child_process_1.execSync)(`git rev-parse ${this.name}`).toString().trim());
    }
    base() {
        var _a;
        ...
        return new commit_1.default((0, child_process_1.execSync)(`git merge-base ${parentBranchName} ${this.name}`)
            .toString()
            .trim());
    }
    getChildrenFromGit() {
        (0, utils_1.logDebug)(`Git Children (${this.name}): start`);
        const kids = (0, git_refs_1.getBranchChildrenOrParentsFromGit)(this, {
            direction: 'children',
            useMemoizedResults: this.shouldUseMemoizedResults,
        });
    ...
    getParentsFromGit() {
        ...
        this.name === (0, utils_1.getTrunk)().name
        // Current branch shares
        ) {
            return [];
        }
        else if (this.pointsToSameCommitAs((0, utils_1.getTrunk)())) {
            return [(0, utils_1.getTrunk)()];
        }
        // In order to tacitly support those that use merge workflows, our logic
        // marks children it has visited - and short circuits - to avoid
        // duplication. This means that the ordering of children must be consistent
        // between git and meta to ensure that our views of their stacks always
        // align.
        return (0, git_refs_1.getBranchChildrenOrParentsFromGit)(this, {
            direction: 'parents',
            useMemoizedResults: this.shouldUseMemoizedResults,
        }).sort(this.sortBranchesAlphabetically);
    }
    pointsToSameCommitAs(branch) {
        return !!(0, git_refs_1.otherBranchesWithSameCommit)(branch).find((b) => b.name === branch.name);
    }
    branchesWithSameCommit() {
        return (0, git_refs_1.otherBranchesWithSameCommit)(this);
    }
    ...
    setPriorReviewers(reviewers) {
        const meta = this.getMeta() || {};
        meta.priorSubmitInfo = Object.assign(Object.assign({}, meta.priorSubmitInfo), { reviewers });
        this.writeMeta(meta);
    }
    getPriorReviewers() {
        var _a, _b;
    ...
    getCommitSHAs() {
        ...
        const shas = new Set();
        const commits = (0, utils_1.gpExecSync)({
