...
class Branch {
    ...
    static calculateMemoizedMetaChildren(context) {
        context.splog.logDebug(`Meta Children: initialize memoization | finding all branches...`);
        const metaChildren = {};
        const allBranches = Branch.allBranches(context, {
            useMemoizedResults: true,
        });
        context.splog.logDebug(`Meta Children: intiialize memoization | sifting through branches...`);
        allBranches.forEach((branch, i) => {
            context.splog.logDebug(`               Branch ${i}/${allBranches.length} (${branch.name})`);
