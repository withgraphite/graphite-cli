...
class AbstractStackBuilder {
    constructor(opts) {
        ...
        this.fullStackFromBranch = (b, context) => {
            const branch = this.memoizeBranchIfNeeded(b);
            splog_1.logDebug(`Finding base branch of stack with ${branch.name}`);
            const base = this.getStackBaseBranch(branch, { excludingTrunk: true }, context);
            splog_1.logDebug(`Finding rest of stack from base branch of stack with ${branch.name}`);
    ...
    upstackInclusiveFromBranchWithoutParents(b, context) {
        ...
        do {
            ...
            splog_1.logDebug(`Looking up children for ${curNode.branch.name}...`);
    ...
    allStackBaseNames(context) {
        splog_1.logDebug(`Getting all branches...`);
        const allBranches = branch_1.Branch.allBranches(context, {
            useMemoizedResults: this.useMemoizedResults,
        });
        splog_1.logDebug(`Got ${allBranches.length} branches`);
        splog_1.logDebug(`Getting all stack base names...`);
        ...
        splog_1.logDebug(`Got ${uniqueStackBaseNames.length} stack base names...`);
    ...
    getStackBaseBranch(branch, opts, context) {
        splog_1.logDebug(`Getting base branch for ${branch.name}...`);
        ...
        if (!parent || ((opts === null || opts === void 0 ? void 0 : opts.excludingTrunk) && parent.isTrunk(context))) {
            splog_1.logDebug(`Base branch is ${branch.name}`);
            return branch;
        }
