...
function rebaseUpstack(context) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            context.splog.logWarn('Cannot fix upstack automatically, some uncommitted changes remain. Please commit or stash, and then `gt upstack fix --rebase`');
        }
    });
}
...
function restackUpstack(args, context) {
    ...
    if (rebased) {
        context.splog.logInfo(`Fixed (${chalk_1.default.green(branch.name)}) on (${parentBranch.name})`);
    }
...
function regenAllStacks(context) {
    ...
    context.splog.logInfo(`Computing regenerating ${allGitStacks.length} stacks...`);
...
function recursiveRegen(node, context) {
    ...
    if (branch.name !== trunk_1.getTrunk(context).name) {
        ...
        if (oldParent && oldParent.name === newParent.name) {
            context.splog.logInfo(`-> No change for (${branch.name}) with branch parent (${oldParent.name})`);
            // Stacks are valid, we can update parentRevision
            // TODO: Remove after migrating validation to parentRevision
            ...
            context.splog.logInfo(`-> Updating (${branch.name}) branch parent from (${oldParent === null || oldParent === void 0 ? void 0 : oldParent.name}) to (${chalk_1.default.green(newParent.name)})`);
            branch.setParentBranch(newParent);
        }
    }
