...
function rebaseUpstack(context) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            utils_1.logWarn('Cannot fix upstack automatically, some uncommitted changes remain. Please commit or stash, and then `gt upstack fix --rebase`');
        }
    });
}
...
function fixAction(opts, context) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const { metaStack, gitStack } = validate_1.getStacksForValidation(opts.scope, context);
        // Consider noop
...
function restackUpstack(args, context) {
    ...
    if (parentBranch.ref(context) === mergeBase) {
        utils_1.logInfo(`No fix needed for (${branch.name}) on (${parentBranch.name})`);
    }
    else {
        utils_1.logInfo(`Fixing (${chalk_1.default.green(branch.name)}) on (${parentBranch.name})`);
        utils_1.checkoutBranch(branch.name, { quiet: true });
        branch.savePrevRef();
        utils_1.gpExecSync({
            ...
            if (utils_1.rebaseInProgress()) {
                throw new errors_1.RebaseConflictError(`Interactive rebase in progress, cannot fix (${branch.name}) onto (${parentBranch.name}).`, args.mergeConflictCallstack, context);
            }
        });
    ...
    if (branch.getParentBranchSha() !== parentBranch.getCurrentRef()) {
        utils_1.logDebug(`Updating parent revision`);
