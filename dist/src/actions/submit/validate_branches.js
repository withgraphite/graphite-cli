...
function getValidBranchesToSubmit(scope, context) {
    return __awaiter(this, void 0, void 0, function* () {
        splog_1.logInfo(chalk_1.default.blueBright(`✏️  [Step 1] Validating that this Graphite stack is ready to submit...`));
...
function hasAnyMergedBranches(branchesToSubmit) {
    ...
    splog_1.logError(`PR${hasMultipleBranches ? 's' : ''} for the following branch${hasMultipleBranches ? 'es have' : ' has'} already been merged:`);
    mergedBranches.forEach((b) => splog_1.logError(`▸ ${chalk_1.default.reset(b.name)}`));
    splog_1.logError(`If this is expected, you can use 'gt repo sync' to delete ${hasMultipleBranches ? 'these branches' : 'this branch'} locally and restack dependencies.`);
    return true;
}
function hasAnyClosedBranches(branchesToSubmit) {
    ...
    splog_1.logError(`PR${hasMultipleBranches ? 's' : ''} for the following branch${hasMultipleBranches ? 'es have' : ' has'} been closed:`);
    closedBranches.forEach((b) => splog_1.logError(`▸ ${chalk_1.default.reset(b.name)}`));
    splog_1.logError(`To submit ${hasMultipleBranches ? 'these branches' : 'this branch'}, please reopen the PR remotely.`);
    return true;
}
function checkForEmptyBranches(submittableBranches, context) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        splog_1.logWarn(`The following branch${hasMultipleBranches ? 'es have' : ' has'} no changes:`);
        emptyBranches.forEach((b) => splog_1.logWarn(`▸ ${chalk_1.default.reset(b.name)}`));
        splog_1.logWarn(`Are you sure you want to submit ${hasMultipleBranches ? 'them' : 'it'}?`);
        splog_1.logNewline();
