...
const graphite_cli_routes_1 = __importDefault(require("graphite-cli-routes"));
...
function submitAction(args) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const cliAuthToken = (0, preconditions_1.cliAuthPrecondition)();
        if (args.dryRun) {
            (0, utils_1.logInfo)(chalk_1.default.yellow(`Running submit in 'dry-run' mode. No branches will be pushed and no PRs will be opened or updated.`));
            (0, utils_1.logNewline)();
            args.editPRFieldsInline = false;
        }
        if (!config_1.execStateConfig.interactive()) {
            (0, utils_1.logInfo)(`Running in interactive mode. All new PRs will be created as draft and PR fields inline prompt will be silenced`);
            args.editPRFieldsInline = false;
        ...
        if (args.branchesToSubmit) {
            ...
            // Step 1: Validate
            (0, utils_1.logInfo)(chalk_1.default.blueBright(`✏️  [Step 1] Validating that this Graphite stack is ready to submit...`));
        ...
        // Step 2: Prepare
        (0, utils_1.logInfo)(chalk_1.default.blueBright('🥞 [Step 2] Preparing to submit PRs for the following branches...'));
        ...
        if (args.dryRun) {
            (0, utils_1.logInfo)(chalk_1.default.blueBright('✅ Dry Run complete.'));
            return;
        }
        // Step 3: Pushing branches to remote
        (0, utils_1.logInfo)(chalk_1.default.blueBright('➡️  [Step 3] Pushing branches to remote...'));
        const branchesPushedToRemote = pushBranchesToRemote(submissionInfoWithBranches.map((info) => info.branch));
        (0, utils_1.logInfo)(chalk_1.default.blueBright(`📂 [Step 4] Opening/updating PRs on GitHub for pushed branches...`));
        ...
        (0, utils_1.logNewline)();
        const survey = yield (0, survey_1.getSurvey)();
        if (survey) {
            yield (0, survey_1.showSurvey)(survey);
        }
    });
}
...
function getValidBranchesToSubmit(scope) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            if (scope === 'BRANCH') {
                const currentBranch = (0, preconditions_1.currentBranchPrecondition)();
                branchesToSubmit = [currentBranch];
            }
            else {
                const stack = getStack({
                    currentBranch: (0, preconditions_1.currentBranchPrecondition)(),
                    scope: scope,
                });
                (0, validate_1.validateStack)(scope, stack);
        ...
        yield (0, pr_info_1.syncPRInfoForBranches)(branchesToSubmit);
...
function processBranchesInInvalidState(branches) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (closedBranches.length > 0 || mergedBranches.length > 0) {
            (0, utils_1.logWarn)(`PRs for the following branches in the stack have been closed or merged:`);
            closedBranches.forEach((b) => (0, utils_1.logWarn)(`▸ ${chalk_1.default.reset(b.name)} (closed)`));
            mergedBranches.forEach((b) => (0, utils_1.logWarn)(`▸ ${chalk_1.default.reset(b.name)} (merged)`));
            (0, utils_1.logWarn)(`This can cause unexpected issues.`);
            if (!config_1.execStateConfig.interactive()) {
                abort = true;
                (0, utils_1.logInfo)(`Aborting.`);
            }
            else {
...
function submitPullRequests(args) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!args.submissionInfoWithBranches.length) {
            (0, utils_1.logInfo)(`No eligible branches to create PRs for.`);
            (0, utils_1.logNewline)();
            return;
        }
...
function getPRInfoForBranches(args) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const branch of args.branches) {
            ...
            else if (previousPRInfo && (0, utils_1.detectUnsubmittedChanges)(branch)) {
                status = 'Update';
            ...
            (0, utils_1.logInfo)(`▸ ${chalk_1.default.cyan(branch.name)} (${status}${reason ? ' - ' + reason : ''})`);
        }
...
function pushBranchesToRemote(branches) {
    ...
    if (!branches.length) {
        (0, utils_1.logInfo)(`No eligible branches to push.`);
        return [];
    }
    branches.forEach((branch) => {
        (0, utils_1.logInfo)(`Pushing ${branch.name} with force-with-lease (will not override external commits to remote)...`);
        const output = (0, utils_1.gpExecSync)({
            ...
            (0, utils_1.logError)(`Failed to push changes for ${branch.name} to remote.`);
            (0, utils_1.logTip)(`There maybe external commits on remote that were not overwritten with the attempted push.
...
function getPRCreationInfo(args) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        (0, utils_1.logInfo)(`Enter info for new pull request for ${chalk_1.default.yellow(args.branch.name)} ▸ ${args.parentBranchName}:`);
        const title = yield (0, pr_title_1.getPRTitle)({
            branch: args.branch,
            editPRFieldsInline: args.editPRFieldsInline,
        });
        args.branch.setPriorSubmitTitle(title);
        const body = yield (0, pr_body_1.getPRBody)({
            branch: args.branch,
            editPRFieldsInline: args.editPRFieldsInline,
        });
        args.branch.setPriorSubmitBody(body);
        const reviewers = yield (0, reviewers_1.getReviewers)({
            fetchReviewers: args.reviewers,
        });
        args.branch.setPriorReviewers(reviewers);
        const createAsDraft = yield (0, pr_draft_1.getPRDraftStatus)({
            createNewPRsAsDraft: args.createNewPRsAsDraft,
        });
...
function printSubmittedPRInfo(prs) {
    if (!prs.length) {
        (0, utils_1.logNewline)();
        (0, utils_1.logInfo)(chalk_1.default.blueBright('✅ All PRs up-to-date on GitHub; no updates necessary.'));
        return;
    }
    prs.forEach((pr) => {
        ...
        if ('error' in pr.response) {
            (0, utils_1.logError)(`Error in submitting ${pr.response.head}: ${pr.response.error}`);
        }
        else {
            (0, utils_1.logSuccess)(`${pr.response.head}: ${chalk_1.default.reset(pr.response.prURL)} ${status}`);
        }
    });
}
