...
function submitAction(args) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            // purposes.
            (0, utils_1.logInfo)(chalk_1.default.blueBright(`✏️  [1/4] Validating Graphite stack before submitting...`));
            if (args.scope !== 'BRANCH') {
                (0, validate_1.validate)(args.scope);
            }
            (0, utils_1.logNewline)();
        }
        catch (_a) {
        ...
        (0, utils_1.logInfo)(chalk_1.default.blueBright('🥞 [2/4] Preparing to submit PRs for the following branches...'));
        branchesToSubmit.forEach((branch) => {
            ...
            (0, utils_1.logInfo)(`▸ ${chalk_1.default.yellow(branch.name)} (${operation})`);
        });
        (0, utils_1.logNewline)();
        if (!args.dryRun) {
...
function pushBranchesToRemote(branches) {
    ...
    (0, utils_1.logInfo)(chalk_1.default.blueBright('➡️  [3/4] Pushing branches to remote...'));
    branches.forEach((branch) => {
        (0, utils_1.logInfo)(`Pushing ${branch.name}...`);
...
function submitPRsForBranches(args) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            (0, utils_1.logInfo)(chalk_1.default.blueBright(`📂 [4/4] Opening/updating PRs on GitHub for pushed branches...`));
...
function shouldUpdatePR(args) {
    ...
    if (getBranchBaseName(args.branch) !== args.previousBranchPRInfo.base) {
        (0, utils_1.logInfo)(chalk_1.default.yellow(`Branch ${args.branch.name} was rebased: will update PR`));
        return true;
    }
    // code was updated
    if (args.branchesPushedToRemote.find((branchPushedToRemote) => branchPushedToRemote.name === args.branch.name)) {
        (0, utils_1.logInfo)(chalk_1.default.yellow(`Code changes detected for ${args.branch.name}: will update PR`));
        return true;
    }
    if (config_1.execStateConfig.outputDebugLogs()) {
        (0, utils_1.logInfo)(`No PR update needed for ${args.branch.name}: PR base and code unchanged.`);
    }
    return false;
}
...
function printSubmittedPRInfo(prs) {
    if (prs.length === 0) {
        (0, utils_1.logInfo)('All PRs up-to-date on GitHub; no PR updates necessary.');
        return;
    }
    prs.forEach((pr) => {
        (0, utils_1.logSuccess)(pr.response.head);
        ...
        if ('error' in pr.response) {
            (0, utils_1.logError)(pr.response.error);
        }
        else {
