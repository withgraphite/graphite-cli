...
function fixDanglingBranches(context, opts) {
    return __awaiter(this, void 0, void 0, function* () {
        splog_1.logInfo(`Ensuring tracked branches in Graphite are all well-formed...`);
        if (opts.showSyncTip) {
            splog_1.logTip(`Disable this behavior at any point in the future with --no-show-dangling`, context);
        }
        ...
        if (danglingBranches.length === 0) {
            splog_1.logInfo(`All branches well-formed.`);
            splog_1.logNewline();
            return;
        }
        splog_1.logNewline();
        ...
        splog_1.logTip(`To ensure Graphite always has a known parent for your branch, create your branch through Graphite with \`gt branch create <branch_name>\`.`, context);
        splog_1.logNewline();
        ...
        for (const branch of danglingBranches) {
            ...
            if (opts.force) {
                ...
                splog_1.logInfo(`Setting parent of ${branch.name} to ${trunk}.`);
            }
            else if (!exec_state_config_1.execStateConfig.interactive()) {
                fixStrategy = 'no_fix';
                splog_1.logInfo(`Skipping fix in non-interactive mode. Use '--force' to set parent to ${trunk}).`);
            }
