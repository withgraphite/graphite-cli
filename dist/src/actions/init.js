...
function init(context, trunk, ignoreBranches) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (allBranches.length === 0) {
            splog_1.logError(`Ouch! We can't setup Graphite in a repo without any branches -- this is likely because you're initializing Graphite in a blank repo. Please create your first commit and then re-run your Graphite command.`);
            splog_1.logNewline();
        ...
        if (trunk) {
            if (branch_exists_1.branchExists(trunk)) {
                ...
                splog_1.logInfo(`Trunk set to (${newTrunkName})`);
            }
            else {
        ...
        if (ignoreBranches) {
            ...
            splog_1.logInfo(`Selected following branches to ignore: ${ignoreBranches}`);
            if (!ignoreBranches) {
                ignoreBranches = [];
            }
            context.repoConfig.addIgnoreBranchPatterns(ignoreBranches);
        }
        splog_1.logInfo(`Graphite repo config saved at "${context.repoConfig.path}"`);
        splog_1.logInfo(fs_extra_1.default.readFileSync(context.repoConfig.path).toString());
    });
}
exports.init = init;
function logWelcomeMessage(context) {
    if (!context.repoConfig.graphiteInitialized()) {
        splog_1.logInfo('Welcome to Graphite!');
    }
    else {
        splog_1.logInfo(`Regenerating Graphite repo config (${context.repoConfig.path})`);
    }
}
