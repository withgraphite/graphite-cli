...
const branch_exists_1 = require("../lib/git/branch_exists");
...
function init(context, trunk, ignoreBranches) {
    return __awaiter(this, void 0, void 0, function* () {
        preconditions_1.getRepoRootPathPrecondition();
        ...
        if (allBranches.length === 0) {
            context.splog.logError(`Ouch! We can't setup Graphite in a repo without any branches -- this is likely because you're initializing Graphite in a blank repo. Please create your first commit and then re-run your Graphite command.`);
            context.splog.logNewline();
        ...
        let newTrunkName;
        if (trunk) {
            if (branch_exists_1.branchExists(trunk)) {
                newTrunkName = trunk;
        ...
        if (ignoreBranches) {
            ...
            context.splog.logInfo(`Selected following branches to ignore: ${ignoreBranches}`);
            if (!ignoreBranches) {
                ignoreBranches = [];
            }
            context.repoConfig.addIgnoreBranchPatterns(ignoreBranches);
        }
        context.splog.logInfo(`Graphite repo config saved at "${context.repoConfig.path}"`);
        context.splog.logInfo(fs_extra_1.default.readFileSync(context.repoConfig.path).toString());
    });
}
exports.init = init;
function logWelcomeMessage(context) {
    if (!context.repoConfig.graphiteInitialized()) {
        ...
        context.splog.logInfo(`Regenerating Graphite repo config (${context.repoConfig.path})`);
    }
}
