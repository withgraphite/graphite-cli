...
function mergeDownstack(branchName, context) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        utils_1.logInfo(`Syncing branch ${chalk_1.default.yellow(branchName)} from remote ${remote}:`);
        const remoteParent = getRemoteParentOrThrow(branchName, remote);
        if (remoteParent !== utils_1.getTrunk(context).name) {
            utils_1.logInfo(`${chalk_1.default.yellow(branchName)} depends on ${chalk_1.default.yellow(remoteParent)}...`);
            if ((yield mergeDownstack(remoteParent, context)) === 'ABORT') {
                return 'ABORT';
            }
        }
        if (!branch_1.Branch.exists(branchName)) {
            utils_1.logInfo(`${chalk_1.default.yellow(branchName)} does not exist locally; no merge needed.`);
            branch_1.Branch.copyFromRemote(branchName, remote);
            utils_1.logInfo(`${chalk_1.default.green(branchName)} set to match remote.`);
            return 'CONTINUE';
        }
...
function getRemoteParentOrThrow(branchName, remote) {
    ...
    if (!branch_1.Branch.metaExistsOnRemote(branchName, remote)) {
        throw new errors_1.ExitFailedError([
            `Metadata for ${chalk_1.default.red(branchName)} does not exist on remote ${remote}.`,
...
function handleExistingBranch(branchName, remote) {
    return __awaiter(this, void 0, void 0, function* () {
        utils_1.logInfo(`${chalk_1.default.yellow(branchName)} exists locally. Merging local state is not yet implemented.`);
