...
function testPRTemplates(args, scene) {
    ...
    args.nonTemplatePaths?.forEach((nonTemplate) => createFile(path_1.default.join(scene.repo.dir, nonTemplate)));
    const foundPRTemplates = scene.repo.execCliCommandAndGetOutput('repo pr-templates');
    args.templatePaths.forEach((template) => (0, chai_1.expect)(foundPRTemplates.includes(template)).to.be.true);
    args.nonTemplatePaths?.forEach((nonTemplate) => (0, chai_1.expect)(foundPRTemplates.includes(nonTemplate)).to.be.false);
}
