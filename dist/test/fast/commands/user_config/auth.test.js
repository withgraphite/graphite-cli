...
for (const scene of [new scenes_1.BasicScene()]) {
    describe(`(${scene}): auth`, function () {
        ...
        it('Sanity check - can read previously written auth token', () => {
            ...
            (0, chai_1.expect)(() => scene.repo.execCliCommand(`auth -t ${authToken}`)).to.not.throw(Error);
            (0, chai_1.expect)(scene.repo.execCliCommandAndGetOutput(`auth`)).to.equal(authToken);
        });
        it('Overwrites any previously stored auth token', () => {
            ...
            (0, chai_1.expect)(() => scene.repo.execCliCommand(`auth -t ${authTokenOld}`)).to.not.throw(Error);
            (0, chai_1.expect)(() => scene.repo.execCliCommand(`auth -t ${authTokenNew}`)).to.not.throw(Error);
            (0, chai_1.expect)(scene.repo.execCliCommandAndGetOutput(`auth`)).to.equal(authTokenNew);
        });
        after(function () {
