...
for (const scene of scenes_1.allScenes) {
    describe(`(${scene}): branch children`, function () {
        ...
        it('Can list children in a stack', () => {
            ...
            scene.repo.checkoutBranch('main');
            (0, chai_1.expect)(scene.repo.execCliCommandAndGetOutput(`branch children`)).to.eq(`a\nb`);
        });
        it('Can list no children', () => {
            (0, chai_1.expect)(scene.repo.execCliCommandAndGetOutput(`branch children`)).to.eq('(main) has no child branches (branches stacked on top of it).');
        });
    });
}
