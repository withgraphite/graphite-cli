...
for (const scene of [new scenes_1.TrailingProdScene()]) {
    describe(`(${scene}): repo ignored-branches`, function () {
        ...
        it('Can add to ignored-branches', () => {
            ...
            (0, chai_1.expect)(savedConfig['ignoreBranches']).to.contain(branchToAdd);
        });
        it('Can remove from ignored-branches', () => {
            ...
            (0, chai_1.expect)(savedConfig['ignoreBranches']).to.contain(branch);
            ...
            (0, chai_1.expect)(savedConfig['ignoreBranches']).to.not.contain(branch);
        });
        it('Can get ignored-branches', () => {
            ...
            (0, chai_1.expect)(scene.repo.execCliCommandAndGetOutput(`repo ignored-branches`)).to.contain(branchToAdd);
            const savedConfig = JSON.parse(fs_extra_1.default.readFileSync(repoConfigPath).toString());
            (0, chai_1.expect)(savedConfig['ignoreBranches'][0]).to.eq(branchToAdd);
        });
    });
}
