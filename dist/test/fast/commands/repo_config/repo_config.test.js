...
for (const scene of [new scenes_1.BasicScene()]) {
    describe(`(${scene}): infer repo owner/name`, function () {
        ...
        it('Can infer cloned repos', () => {
            const { owner, name } = repo_config_1.getOwnerAndNameFromURLForTesting('https://github.com/screenplaydev/graphite-cli.git');
            chai_1.expect(owner === 'screenplaydev').to.be.true;
            chai_1.expect(name === 'graphite-cli').to.be.true;
        });
        it('Can infer SSH cloned repos', () => {
            const { owner, name } = repo_config_1.getOwnerAndNameFromURLForTesting('git@github.com:screenplaydev/graphite-cli.git');
            chai_1.expect(owner === 'screenplaydev').to.be.true;
            chai_1.expect(name === 'graphite-cli').to.be.true;
        });
        ...
        it('Can infer cloned repos without .git', () => {
            const clone = repo_config_1.getOwnerAndNameFromURLForTesting('https://github.com/screenplaydev/graphite-cli');
            chai_1.expect(clone.owner === 'screenplaydev').to.be.true;
            chai_1.expect(clone.name === 'graphite-cli').to.be.true;
            const sshClone = repo_config_1.getOwnerAndNameFromURLForTesting('git@github.com:screenplaydev/graphite-cli');
