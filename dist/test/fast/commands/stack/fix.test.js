...
for (const scene of scenes_1.allScenes) {
    ...
    describe(`(${scene}): stack fix`, function () {
        ...
        it('Can fix a stack of three branches', () => {
            ...
            (0, utils_1.expectCommits)(scene.repo, '4, 3.5, 3, 2.5, 2, 1');
            ...
            (0, utils_1.expectCommits)(scene.repo, '4, 3.5, 3, 2.5, 2, 1.5, 1');
        });
        ...
        it('Can fix one specific stack', () => {
            ...
            scene.repo.execCliCommand('stack fix --rebase -q');
            (0, chai_1.expect)(scene.repo.currentBranchName()).to.eq('b');
            (0, utils_1.expectCommits)(scene.repo, 'b, a, 1.5, 1');
        });
        // regen tests
        it('Can regen a stack from scratch', () => {
            ...
            (0, chai_1.expect)(branch.stackByTracingMetaParents().join(',')).not.to.equal(branch.stackByTracingGitParents().join(','));
            scene.repo.checkoutBranch('a');
            scene.repo.execCliCommand('stack fix --regen -q');
            scene.repo.checkoutBranch('b');
            (0, chai_1.expect)(branch.stackByTracingMetaParents().join(',')).to.equal(branch.stackByTracingGitParents().join(','));
        });
        ...
        it('Expect a branch matching main to throw an error', () => {
            ...
            (0, chai_1.expect)(() => scene.repo.execCliCommand('stack fix --regen -q')).to.throw(Error);
        });
