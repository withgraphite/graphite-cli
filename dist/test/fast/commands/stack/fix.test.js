...
for (const scene of scenes_1.allScenes) {
    ...
    describe(`(${scene}): stack fix`, function () {
        ...
        it('Can regen a stack from scratch', () => {
            ...
            chai_1.expect(branch.stackByTracingMetaParents(context).join(',')).not.to.equal(branch.stackByTracingGitParents(context).join(','));
            scene.repo.checkoutBranch('a');
            scene.repo.execCliCommand('stack fix --regen -q');
            scene.repo.checkoutBranch('b');
            chai_1.expect(branch.stackByTracingMetaParents(context).join(',')).to.equal(branch.stackByTracingGitParents(context).join(','));
        });
