...
for (const scene of all_scenes_1.allScenes) {
    describe(`(${scene}): commit create continue`, function () {
        ...
        it('Can run continue multiple times on a commit create with multiple merge conflicts', () => {
            ...
            // should not be lost.
            chai_1.expect(scene.repo.currentBranchName()).to.equal('a');
            expect_commits_1.expectCommits(scene.repo, 'a12, a, 1');
            chai_1.expect(scene.repo.rebaseInProgress()).to.be.false;
            scene.repo.checkoutBranch('c');
            expect_commits_1.expectCommits(scene.repo, 'c, b, a12, a');
        });
    });
}
