...
for (const scene of scenes_1.allScenes) {
    ...
    describe(`(${scene}): stack builder class`, function () {
        ...
        it('Can print stacks from git', () => {
            ...
            const gitStacks = new wrapper_classes_1.GitStackBuilder().allStacks(context);
            const metaStacks = new wrapper_classes_1.MetaStackBuilder().allStacks(context);
        ...
        it('Can get full stack from a branch', () => {
            ...
            scene.repo.createChange('d');
            scene.repo.execCliCommand(`branch create "d" -m "d" -q`);
            const metaStack = new wrapper_classes_1.MetaStackBuilder().fullStackFromBranch(new branch_1.default('a'), context);
            const gitStack = new wrapper_classes_1.GitStackBuilder().fullStackFromBranch(new branch_1.default('a'), context);
        ...
        it('Can get full stack from trunk', () => {
            ...
            scene.repo.createChange('d');
            scene.repo.execCliCommand(`branch create "d" -m "d" -q`);
            const metaStack = new wrapper_classes_1.MetaStackBuilder().fullStackFromBranch(new branch_1.default('main'), context);
            const gitStack = new wrapper_classes_1.GitStackBuilder().fullStackFromBranch(new branch_1.default('main'), context);
        ...
        it('Can get just downstack from a branch', () => {
            ...
            const metaStack = new wrapper_classes_1.MetaStackBuilder().downstackFromBranch(new branch_1.default('b'), context);
            const gitStack = new wrapper_classes_1.GitStackBuilder().downstackFromBranch(new branch_1.default('b'), context);
            chai_1.expect(metaStack.equals(wrapper_classes_1.Stack.fromMap({ main: { a: { b: {} } } }))).to.be
                .true;
            chai_1.expect(metaStack.equals(gitStack)).to.be.true;
        });
        it('Can get branchs from a stack', () => {
            ...
            scene.repo.createChange('c');
            scene.repo.execCliCommand(`branch create "c" -m "c" -q`);
            const metaStack = new wrapper_classes_1.MetaStackBuilder().fullStackFromBranch(new branch_1.default('b'), context);
