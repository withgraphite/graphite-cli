...
- Fixed a shebang issue that resulted from trying to clean up NodeJS v18 warnings. Added a different fix to prevent node warnings from showing up in CLI output.
- Fixed a bug with NodeJS v14 support.
- Changed the Homebrew NodeJS dependency to v16, which is the same as we use for development.
